...
const ForgotPassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.post(`${apiBase}/api/forgot-password`, { email: trimmedEmail });
            // Always show the same success state, whether or not that email is
            // registered — the backend intentionally responds the same way either
            // way, so this page shouldn't imply otherwise.
            setSubmitted(true);
        } catch (err) {
                ...
                <div className="bg-white/[0.03] backdrop-blur-3xl border border-white/10 rounded-[3rem] p-8 md:p-10 shadow-2xl shadow-blue-900/30">
                    ...
                    {submitted ? (
                        <div className="space-y-6">
                            ...
                            >
                                ...
                                <p className="text-sm text-emerald-100/80 leading-relaxed">
                                    If an account exists for <span className="text-white font-bold">{email.trim()}</span>,
                            ...
                            <button
                                ...
                                className="w-full py-4 rounded-2xl font-black text-xs uppercase tracking-[0.3em] text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:shadow-[0_0_30px_rgba(37,99,235,0.4)] transition-all active:scale-[0.98]"
                            >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            <p className="text-xs text-white/40 leading-relaxed -mt-2 mb-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative group">
                                    <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-blue-500/30 group-focus-within:text-blue-400 transition-colors" size={18} />
                                    <input
                                        type="email"
                                        value={email}
                            ...
                            <button
                                type="button"
                                onClick={() => navigate('/login')}
                                className="w-full flex items-center justify-center gap-2 text-[10px] font-bold text-blue-400/50 hover:text-blue-400 uppercase tracking-widest transition-colors pt-2"
                            >
