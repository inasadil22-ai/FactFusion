...
const dataProvider = {
    ...
    // 10. CUSTOM MULTIMODAL MEDIA UPLOAD
    analyzeContent: (text, file, userId) => {
        const formData = new FormData();
        if (text) formData.append('text', text);
        if (file) formData.append('file', file);
        if (userId) formData.append('user_id', userId);

        // Axios bypasses the raw fetch utils wrapper to naturally format multi-part stream bounds
        return axios.post(`${apiUrl}/analyze`, formData, {
            headers: {
